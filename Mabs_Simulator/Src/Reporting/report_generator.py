...
class ReportGenerator():
    ...
    def plot_averaged_regret(self, statistics, algorithm_name):
        ...
        ax.plot(iterations, mean_regret, linewidth=2, label=f"{algorithm_name} (moyenne)")
        ax.fill_between(
            iterations,
        ...
        ax.set_title(f"Regret cumulé moyen — {algorithm_name} ({n_runs} runs)", fontsize=14)
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()

        output_path = f"{self.results_path}/averaged_regret_{algorithm_name}.png"
